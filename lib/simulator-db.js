...
export class SimulatorDB {
  ...
  generateSummary() {
    ...
    const totalBalance = activeCapital + netPnL;
    
    // Calculate Effective Win Rate (including open positions)
    const openWins = openPositions.filter(p => (p.unrealizedPnL || 0) >= 0).length;
    const openLosses = openPositions.filter(p => (p.unrealizedPnL || 0) < 0).length;
    ...
    const slPct = config.stopLoss ? ((1 - config.stopLoss) * 100).toFixed(0) : 'None';
    const trailPct = config.trailActivation ? ((config.trailActivation - 1) * 100).toFixed(0) : '50';
    const tdPct = config.trailDistance ? (config.trailDistance * 100).toFixed(0) : '10';

    return `📊 <b>Live Portfolio Status</b>

🏦 <b>Init Capital:</b> $${startingCapital.toLocaleString()} → <b>$${currentBalance.toLocaleString()}</b> (${totalGrowth >= 0 ? '+' : ''}${totalGrowth}%)
🔄 <b>Cum Used:</b> $${totalDeployed.toLocaleString()} (${stats.totalTrades} trades)

💵 <b>Active:</b> $${activeCapital.toFixed(0)} (${openPositions.length} positions)
💸 <b>Unrealized:</b> ${unrealizedPnL >= 0 ? '+' : ''}$${unrealizedPnL.toFixed(2)}
🔥 <b>Ongoing:</b> $${ongoingCapital.toFixed(2)}

💰 <b>Realized:</b> ${stats.totalPnL >= 0 ? '+' : ''}$${stats.totalPnL.toFixed(2)}
📈 <b>Net PnL:</b> ${netPnL >= 0 ? '+' : ''}$${netPnL.toFixed(2)}
⭐️ <b>Total PnL:</b> $${currentBalance.toLocaleString()}

🏆 <b>Win Rate:</b> ${winRate}% (${totalWins}W / ${totalLosses}L)
<b>ROI Cum:</b> ${roiCum}% | <b>ROI Init:</b> ${roiInit}%
⚙️ <b>Strategy:</b> SL -${slPct}% | Trail +${trailPct}% | TD ${tdPct}%
