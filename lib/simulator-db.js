...
export class SimulatorDB {
  ...
  generateSummary() {
    ...
    // Avg Win/Loss
    const avgWin = stats.winCount > 0 ? (stats.totalWinPct / stats.winCount).toFixed(0) : '0';
    const avgLoss = stats.lossCount > 0 ? (stats.totalLossPct / stats.lossCount).toFixed(0) : '0';
    
    // R:R Ratio (Avg Win / Avg Loss)
    const rrRatio = (Math.abs(parseFloat(avgLoss)) > 0) 
      ? (parseFloat(avgWin) / Math.abs(parseFloat(avgLoss))).toFixed(1) 
      : '0.0';
...
⚙️ Strat: SL -${slPct}% | Trail +${trailPct}% | TD ${tdPct}%`;
  ...
  closePosition(tokenAddress, exitPrice, reason) {
    ...
    const roi = (pos.pnl / pos.size) * 100;
    
    if (pos.pnl > 0) {
      this.db.stats.winCount++;
      this.db.stats.totalWinPct = (this.db.stats.totalWinPct || 0) + roi;
    } else {
      this.db.stats.lossCount++;
      this.db.stats.totalLossPct = (this.db.stats.totalLossPct || 0) + roi;
    }
