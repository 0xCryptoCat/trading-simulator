...
🏦 <b>Capital:</b> $${startingCapital.toLocaleString()} → <b>$${currentBalance.toLocaleString()}</b> (${totalGrowth >= 0 ? '+' : ''}${totalGrowth}%)
💵 <b>Active:</b> $${activeCapital.toFixed(0)} (${openPositions.length} positions)
🔄 <b>Total Deployed:</b> $${totalDeployed.toLocaleString()} (${stats.totalTrades} trades)
